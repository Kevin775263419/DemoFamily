...
+(NSURLSessionDataTask *)getWithUrlPath:(NSString *)urlPath request:(id )request success:(HttpSuccess)success failure:(HttpFailure)failure {
    NSString *jsonString = [request modelToJSONString];
    NSDictionary *parameters = [jsonString modelToJSONObject];
...
+(NSURLSessionDataTask *)deleteWithUrlPath:(NSString *)urlPath request:(id )request success:(HttpSuccess)success failure:(HttpFailure)failure {
    NSString *jsonString = [request modelToJSONString];
    NSDictionary *parameters = [jsonString modelToJSONObject];    return [[NetworkHelper sharedManager] DELETE:[BaseUrl stringByAppendingString:urlPath] parameters:parameters success:^(NSURLSessionDataTask *task, id responseObject) {
        [NetworkHelper processResponseData:responseObject success:success failure:failure];
    } failure:^(NSURLSessionDataTask *task, NSError *error) {
        failure(error);
    }];
...
+(NSURLSessionDataTask *)uploadWithUrlPath:(NSString *)urlPath dataArray:(NSArray<NSData *> *)dataArray request:(id )request progress:(UploadProgress)progress success:(HttpSuccess)success failure:(HttpFailure)failure {
    ...
    return [[NetworkHelper sharedManager] POST:[BaseUrl stringByAppendingString:urlPath] parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        for(NSData *data in dataArray) {
            NSString *fileName = [NSString  stringWithFormat:@"%@.jpg",[NSString stringWithFormat:@"%ld",date.hour + date.nanosecond]];

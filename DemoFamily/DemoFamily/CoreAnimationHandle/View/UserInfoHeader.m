...
- (void) initAvatarBackground {
    //SectionHedader top view
    _topBackground = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, 50 + SafeAreaTopHeight)];
    _topBackground.contentMode = UIViewContentModeScaleAspectFill;
    [self addSubview:_topBackground];
    
    //Bottom view
    _bottomBackground = [[UIImageView alloc] initWithFrame:CGRectMake(0, 35 + SafeAreaTopHeight, kScreenWidth, self.bounds.size.height - (35 + SafeAreaTopHeight))];
...
-(UIBezierPath *)createBezierPath {
    ...
    [bezierPath addLineToPoint:CGPointMake(kScreenWidth, topOffset)];
    [bezierPath addLineToPoint:CGPointMake(kScreenWidth, self.bounds.size.height - (50 + SafeAreaTopHeight) + topOffset - 1)];
...
- (void)initData:(User *)user {
    ...
    [_avatar setImageWithURL:[NSURL URLWithString:user.avatar_medium.url_list.firstObject] placeholder:nil options:YYWebImageOptionProgressive completion:^(UIImage * _Nullable image, NSURL * _Nonnull url, YYWebImageFromType from, YYWebImageStage stage, NSError * _Nullable error) {
        [wself.bottomBackground setImage:image];
//        [self.topBackground setImage:image];
        [wself.avatar setImage:[image drawCircleImage]];
    }];
    //新增的背景数据写为指定路径
    [_topBackground setImageURL:[NSURL URLWithString:@"http://pb3.pstatp.com/obj/dbc1001cd29ccc479f7f"]];
